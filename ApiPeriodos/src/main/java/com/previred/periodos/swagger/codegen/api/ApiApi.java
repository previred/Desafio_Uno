...
public interface ApiApi {
    ...
    @RequestMapping(value = "/api/getRandomPeriods",
    ...
    @ApiOperation(value = "Lista de periodos a procesar", nickname = "periodosFaltantes", notes = "", response = PeriodosFaltantes.class, tags={ "periodosFaltantes", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Periodo y lista de fechas", response = PeriodosFaltantes.class) })
    @RequestMapping(value = "/api/getAllPeriodsByRandom",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    default ResponseEntity<PeriodosFaltantes> periodosFaltantes() {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("\"\"", PeriodosFaltantes.class), HttpStatus.NOT_IMPLEMENTED);
                } catch (IOException e) {
                    log.error("Couldn't serialize response for content type application/json", e);
                    return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
                }
