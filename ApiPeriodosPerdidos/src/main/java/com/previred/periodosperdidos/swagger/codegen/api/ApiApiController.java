...
import com.previred.periodosperdidos.servicio.PeriodosService;
...
public class ApiApiController implements ApiApi {
    ...
    @Override
    public ResponseEntity<PeriodoPerdidoRest> periodosPerdidos() {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    PeriodoPerdidoRest detalle = periodosService.getPeriodosPerdidos();
                    ResponseEntity<PeriodoPerdidoRest> respuesta = new ResponseEntity<>(detalle, HttpStatus.OK);
                    return respuesta;
                } catch (Exception e) {
