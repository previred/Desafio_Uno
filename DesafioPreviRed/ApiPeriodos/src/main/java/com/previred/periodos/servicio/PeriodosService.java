...
public class PeriodosService {
    ...
    public Periodo getFaltantes(Periodo periodo) {
             
       LocalDate fechaI=periodo.getFechaCreacion();
       LocalDate fechaF=periodo.getFechaFin();
       List<LocalDate> fechasEntrantes  = periodo.getFechas();
       Set<LocalDate> fechasFaltantes = new HashSet();
       boolean b = true;

       while (b) {    	  
    	   if(!fechasEntrantes.contains(fechaI)) {
    		   fechasFaltantes.add(fechaI);
    	   }    	   
    	   if(fechaI.isEqual(fechaF)) {
    		   b=false;
    	   }    	
    	   
    	   fechaI = fechaI.plusMonths(1);  	   
       }
